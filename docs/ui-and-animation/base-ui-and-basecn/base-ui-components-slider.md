...
- **Form controls must have an accessible name**: The thumb must have a meaningful label. Prefer using [`<Field>`](/react/components/field.md) to provide visible text labels and descriptions, or [`<Fieldset>`](/react/components/fieldset.md) for multi-thumb range sliders. Otherwise, the `aria-label` attribute can be used as an alternative. See the [forms guide](/react/handbook/forms.md) for more on building form controls.
